...
本章主要对 MyBatis 框架进行了讲解。首先对框架的概念、优点和当前一些主流的 JavaEE 框架进行了讲解，然后对传统 JDBC 的劣势进行了分析，由此引出了 MyBatis 框架，并对 MyBatis 框架的环境搭建、入门程序和工作原理进行了详细讲解。通过学习本章的内容，读者可以了解 MyBatis 框架及其作用，熟悉 MyBatis 的工作原理，并能够独立完成 MyBatis 框架环境的搭建和入门程序的编写。
